     ...
     if (IsData == true) {
      ...
      for (int ijet = 0; ijet < Mjet.JetCount; ijet++) {
       ...
       // add the basic jet variables to the tree
        MDzeroJetUPC.RawJetPt->push_back(Mjet.JetRawPT[ijet]);
	  ...
	  double minDistance = 9999;
	  int d0Index = -1;
	  for(int id = 0; id < MDzeroJetUPC.Gpt->size(); id++){
            // calculate the distance
            double deltaYsq = (Mjet.GenY[igen] - MDzeroJetUPC.Gy->at(id)) *  (Mjet.GenY[igen] - MDzeroJetUPC.Gy->at(id));
            double deltaPhisq = (Mjet.GenPhi[igen] - MDzeroJetUPC.Gphi->at(id)) * (Mjet.GenPhi[igen] - MDzeroJetUPC.Gphi->at(id));
            double dist = sqrt(deltaYsq + deltaPhisq);
            if(dist < minDistance) {
              minDistance = dist;
              d0Index = id;
          ...
          // if after looping over all of the D0s we found one within the cone, fill the jet pt
	  if(minDistance < D0MathchingDistance){
	    MDzeroJetUPC.isD0TaggedGeomGenJet->push_back(true);
	    MDzeroJetUPC.TaggedLeadingD0GeomInGenJetIndex->push_back(d0Index);
	  }
	  else{
	    MDzeroJetUPC.isD0TaggedGeomGenJet->push_back(false);
	    MDzeroJetUPC.TaggedLeadingD0GeomInGenJetIndex->push_back(-1);
	  }
	  ...
	  // note this tagging is before any pT cut of the D0
          ...
          for(int id = 0; id < MDzeroJetUPC.Gpt->size(); id++){
            // calculate the distance
            double deltaYsq = (Mjet.RefY[ijet] - MDzeroJetUPC.Gy->at(id)) *  (Mjet.RefY[ijet] - MDzeroJetUPC.Gy->at(id));
            double deltaPhisq = (Mjet.RefPhi[ijet] - MDzeroJetUPC.Gphi->at(id)) * (Mjet.RefPhi[ijet] - MDzeroJetUPC.Gphi->at(id));
            double dist = sqrt(deltaYsq + deltaPhisq);
            if(dist < minDistance) {
              minDistance = dist;
              d0Index = id;
          ...
          // if after looping over all of the D0s we found one within the cone, fill the jet pt
          if(minDistance < D0MathchingDistance){
            MDzeroJetUPC.isD0TaggedGeomRefJet->push_back(true);
            MDzeroJetUPC.TaggedLeadingD0GeomInRefJetIndex->push_back(d0Index);
          }
          else{
            MDzeroJetUPC.isD0TaggedGeomRefJet->push_back(false);
            MDzeroJetUPC.TaggedLeadingD0GeomInRefJetIndex->push_back(-1);
          }
