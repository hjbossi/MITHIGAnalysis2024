    ...
    for (int iE = 0; iE < EntryCount; iE++) {
     ...
     if (IsData == true) {
      ...
      for (int iD = 0; iD < MDzero.Dsize; iD++) {
        if (ApplyDRejection == 1 && DmesonSelectionPrelim23(MDzero, iD) == false) continue;
        countSelDzero++;
        ...
        MDzeroJetUPC.DpassCut23PAS->push_back(DmesonSelectionPrelim23(MDzero,iD));
        if (IsData == false) {
       ...
       // add the basic jet variables to the tree
        ...
        for(int id = 0; id < MDzeroJetUPC.Dpt->size(); id++){
         ...
         if(minDistance < D0MathchingDistance){
          MDzeroJetUPC.isD0Tagged->push_back(true);
          MDzeroJetUPC.TaggedD0Index->push_back(d0Index);
         }
         else{
          MDzeroJetUPC.isD0Tagged->push_back(false);
          MDzeroJetUPC.TaggedD0Index->push_back(-1);
         }
